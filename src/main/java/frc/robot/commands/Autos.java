...
public class Autos {
  ...
  public AutoRoutine elvis(java.util.function.Supplier<Command> shootCommand, java.util.function.Supplier<Command> stopShootingCommand) {
    ...
    eTraj.atTime("Shoot").onTrue(
      shootCommand.get().withTimeout(3)
    );

    eTraj.atTime("Stop Shooting").onTrue(
      stopShootingCommand.get().withTimeout(3)
    );
